...
_class_name = 'DeployApps'
_SETTINGS_0 = 'settings-0'


class DeployApps(unittest.TestCase):
...
def _populate_tool_args(args, settings):
    ...
    if settings.is_field_set(TestDefSettings.VARIABLE_FILE):
        ...
        args.append('%s' % settings.get_variable_file_name())

    if settings.is_field_set(TestDefSettings.WLST_PATH):
        args.append('-%s' % TestDefSettings.WLST_PATH)
        args.append('%s' % settings.get_wlst_path())

    if settings.is_field_set(TestDefSettings.PREV_MODEL_FILE):
        args.append('-%s' % TestDefSettings.PREV_MODEL_FILE)
        args.append('%s' % settings.get_prev_model_file_name())
